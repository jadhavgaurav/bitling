...
Antigravity, Gemini, or a human). More than one of those may be editing this tree at the
same time — see "Working alongside other agents" below before touching a shared file.
...
- Prefer small, anchored patches (find an exact string, replace it) over a blind full-file
  rewrite, and use a **unique** anchor. A non-unique `sub()`/replace call has silently
  corrupted a different species' block in this file before, by matching more than one
  location. After any patch, diff the whole file against the last known-good commit, not just
  the block you meant to touch, to catch that class of mistake.
- Never `git checkout`, `reset`, or otherwise discard uncommitted changes in this tree
  without checking `git status` first and understanding whose work is there.
