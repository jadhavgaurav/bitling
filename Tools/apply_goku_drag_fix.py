...
def run_fix():
    ...
    start_marker = "  function drawGoku() {"
    end_marker = "  // ---------------------------------------------------------------- Nimbo, the cloud rider"

    idx_start = content.find(start_marker)
    idx_end = content.find(end_marker)

    assert idx_start != -1, f"start_marker {start_marker} not found"
    assert idx_end != -1, f"end_marker {end_marker} not found"
    assert idx_start < idx_end, "markers in wrong order"

    new_draw_goku = """  function drawGoku() {
    ...
    stageBody({ shadow: false, bob: isCarried ? 0.01 : 0.05, bobRate: 2.2, sway: isCarried ? 0 : 0.02 }, (r, t) => {
      ...
      if (isFlying) {
        ...
        ctx.save();
        // Golden speed streamer aligned directly behind the Nimbus cloud tail
...
"""
    ...
    content = content[:idx_start] + new_draw_goku + "\n" + content[idx_end:]
    bitling_path.write_text(content, encoding="utf-8")
    print(f"Updated {bitling_path} successfully!")
